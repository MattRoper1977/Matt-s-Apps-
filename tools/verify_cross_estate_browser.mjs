    ...
    try{
      ...
      if(width===430&&metrics.cards>0){
        if(kind==='lessons'){
          ...
          check(await page.locator('#groups .card p').evaluateAll(es=>es.filter(e=>e.getClientRects().length).every(e=>{const s=getComputedStyle(e),h=e.getBoundingClientRect().height,line=parseFloat(s.lineHeight);return h>=line-1&&h<=line+1&&s.visibility==='visible'&&Number(s.opacity)>0})),'SW2 A descriptions stay on one visual line with full manifest text retained');
          }
